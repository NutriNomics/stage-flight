...
def resolve_filepath(filename):
    """Resolve the file path. If not absolute, look in the 'data' folder."""
    if os.path.isabs(filename):
        return filename

    # Check in the 'data' folder within the project directory
    library_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(library_dir)
    data_folder = os.path.join(project_dir, 'data')
    potential_path = os.path.join(data_folder, filename)
...
class Updater:
    def __init__(self, zip_path="update.zip"):
        ...
        resolve_filepath("version")
        self.version_file = resolve_filepath("version")
